...
class MainMenu:
    def __init__(self, root):
        ...
        self.play_image = self.play_image.resize((203, 75), Image.LANCZOS)  # Resize image if necessary
        self.play_photo = ImageTk.PhotoImage(self.play_image)

        # Add play button with image
        self.play_button = tk.Button(self.root, command=self.start_game, image=self.play_photo, 
        ...
        self.exit_button = tk.Button(self.root, image=self.exit_photo, command=self.root.quit, 
                                    bd=0, highlightthickness=0, highlightcolor="#1f6eb1", activebackground="#1f6eb1")
